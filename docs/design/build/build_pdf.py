...
def screen_slide(c, page_no, index, total, title, tag, tag_color, image_name, points, layout="right"):
    """layout='right': image left ~62%, annotation column right.
    layout='bottom': image full-width top, annotation strip bottom (for very wide/short content)."""
    slide_bg(c)
    ...
    bw = badge(c, PAGE_W - MARGIN - 130, PAGE_H - 0.72 * 72, tag, tag_color)
    ...
    if layout == "right":
        img_w = PAGE_W * 0.615 - MARGIN
        image_frame(c, img_path, MARGIN, top_y, img_w, avail_h)

        col_x = MARGIN + img_w + 30
        col_w = PAGE_W - MARGIN - col_x
        cy = top_y - 6
        text(c, col_x, cy, "Why it's built this way", font=SANS_B, size=11.5, color=TEXT)
        cy -= 22
        ...
        img_h = avail_h * 0.66
        image_frame(c, img_path, MARGIN, top_y, img_w, img_h)
        strip_y = top_y - img_h - 14
        col_w = (img_w - 2 * 24) / 3
        cx = MARGIN
        for label, body in points[:3]:
            text(c, cx, strip_y, label, font=SANS_B, size=10, color=TEXT)
            wrapped(c, cx, strip_y - 15, body, size=9, color=TEXT_MUTED, max_width=col_w, leading=12)
            cx += col_w + 24
...
def closing_slide(c, page_no):
    ...
    text(c, PAGE_W - MARGIN, 0.72 * 72, "11 SCREENS · 19 PAGES", font=MONO, size=9, color=TEXT_DIM, align="right")
...
def main():
    ...
    screen_slide(
        ...
        [
            ...
            ("Quality is a number, not a vibe", "Brand safety and goal alignment are QA's own scores, averaged across real runs — the dashboard reports what the agents concluded about their own work, unfiltered."),
        ],
    )
    c.showPage()
    page += 1
    ...
    screen_slide(
        ...
        [
            ...
            ("Revision cycles render as their own step", "Rather than silently re-running the content agent, “revise” gets its own row in the timeline — the loop is a visible event, not something you'd only find by reading logs."),
        ],
    )
    c.showPage()
    page += 1
    ...
    screen_slide(
        ...
        [
            ("Every run is replayable after the fact", "The drawer shows the identical agent trace from the live view — nothing about a run's history is summarised away once it's over."),
    ...
    screen_slide(
        ...
        [
            ("Published, not pending — by default", "Assets land here the moment QA passes them, tagged with their real status. Nothing sits in an approval queue unless the tenant's policy explicitly asks for that."),
    ...
    screen_slide(
        ...
        [
            ("The record that replaces a human approver", "Nobody signs off on a run, so every outcome — published, dropped, abandoned — is written here with a reason. This table is the accountability structure."),
